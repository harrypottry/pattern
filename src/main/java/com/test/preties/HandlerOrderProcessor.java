...
public class HandlerOrderProcessor implements ApplicationContextAware {
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        //获取所有策略注解的Bean
        Map<String, Object> orderStrategyMap = applicationContext.getBeansWithAnnotation(HandlerLabelType.class);
        orderStrategyMap.forEach((k,v)->{
            //获取策略实现类
            Class<OrderStrategy> orderStrategyClass = (Class<OrderStrategy>) v.getClass();
            //获取策略实现类的注解值。
            int type = orderStrategyClass.getAnnotation(HandlerLabelType.class).value();
            //将class加入HandlerOrderContext的map中,type作为key
            HandlerLabelContext.orderStrategyBeanMap.put(type,orderStrategyClass);
        });
    }
}